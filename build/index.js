...
async function salvarPedidoNoBanco(pedido) {
    ...
    try {
        ...
        if (externalId) {
            const [rows] = await connection.execute("SELECT id_order FROM o01_order WHERE external_id_order = ?", [externalId]);
            pedidoExistente = Array.isArray(rows) && rows.length > 0 ? rows[0] : null;
        }
        let resultado;
        if (pedidoExistente) {
            // Atualizar pedido existente
            const [result] = await connection.execute(`UPDATE o01_order SET
        ...
        WHERE external_id_order = ?`, [
            ...
            // Inserir novo pedido
            const [result] = await connection.execute(`INSERT INTO o01_order (
          shortReference_order,
          date_order,
          time_order,
          subtotal_order,
